...
<div id="container">
        ...
        <table width="100%" border=0>
          <tr>
           ...
           <td><select name="selpid" id="selpid" style="width:180px;">
               ...
               <?php
                $get_pid= @mysql_query("SELECT DISTINCT prd_id FROM orders_tbl WHERE username= '$username' AND ord_deliverystatus= 'Waiting'")or die(mysql_error());
          ...
          <tr>
           ...
           <td><select name="seldate" id="seldate" style="width:180px;">
               ...
               <?php
                $get_date= @mysql_query("SELECT DISTINCT ord_odate FROM orders_tbl WHERE ord_deliverystatus= 'Waiting' AND username= '$username' ORDER BY ord_odate DESC ")or die(mysql_error());
                $num_date= @mysql_num_rows($get_date);
                for($i=0;$i<$num_date;$i++)
                {
 ...
 <?php
       ...
       {
	$get= @mysql_query("SELECT * FROM orders_tbl WHERE username='$username' AND prd_id='$dpid' AND ord_deliverystatus= 'Waiting'")or die(mysql_error());
 ...
 <?php
       ...
       {
	$get= @mysql_query("SELECT * FROM orders_tbl WHERE username='$username' AND ord_odate='$ddate' AND ord_deliverystatus= 'Waiting'")or die(mysql_error());
        $numprd = @mysql_num_rows($get);
        for($i=0;$i<$numprd;$i++)
        {
         $row= @mysql_result($get,$i,'row_id');
...
<?php }
       ...
       {
	 $count=@mysql_query("SELECT * from orders_tbl WHERE username='$username' AND prd_id='$dpid' AND ord_odate='$ddate'");
         $order_count=@mysql_num_rows($count);
	if($order_count==0){
 ?>
 <tr>
 ...
 <?php
	...
	$get= @mysql_query("SELECT * FROM orders_tbl WHERE username='$username' AND prd_id='$dpid' AND ord_odate='$ddate' AND ord_deliverystatus= 'Waiting'")or die(mysql_error());
        $numprd = @mysql_num_rows($get);
        for($i=0;$i<$numprd;$i++)
        {
         $row= @mysql_result($get,$i,'row_id');
  ...
  if(isset($_POST['subDel']))
  {
   for($b=0;$b<$numprd;$b++)
   {
    if(isset($_POST['chk'.$b]))
    {
     ...
     $r_id=$_POST['hdnrid'.$b];
     $today=date("Y-m-d");
     $deldate= @mysql_result(@mysql_query("SELECT ord_ddate FROM orders_tbl WHERE username='$username' AND prd_id='$pid' AND row_id='$r_id'"),0,'ord_ddate');
     $deldate=date('Y-m-d',strtotime($deldate));
     if($today >= $deldate)
     {
      $ordupdate=@mysql_query("UPDATE orders_tbl SET ord_deliverystatus='Delivered' WHERE username='$username' AND prd_id='$pid' AND row_id='$r_id'");
      echo "<script>alert('Products Delivered !!');</script>";
      $count_del=@mysql_query("SELECT * from orders_tbl WHERE username='$username' AND ord_deliverystatus= 'Waiting'");
      $del_count=@mysql_num_rows($count_del);
      if($del_count==0)
      {
