...
<div id="mainContent">
 ...
 <?php
  if(isset($_POST['submitMain']))
  {
   ...
   {
    $get= @mysql_query("SELECT * FROM orders_tbl WHERE username='$username' AND ord_deliverystatus='Delivered'");
    for($i=0;$i<$get;$i++)
    {
     ...
     // Qurey insert into back up table & delete from order table
      $query = mysql_query("INSERT INTO backup_trn_tbl (prd_id,username,bck_archive,bck_pname,bck_qty,bck_price,bck_fname,bck_lname,bck_odate,bck_ddate,bck_email,bck_baddress,bck_saddress,bck_country,bck_mobile,bck_phone)
                           VALUES ('$prid','$username','$today','$pname','$oqty','$price','$fname','$lname','$odate','$ddate','$mail','$badd','$sadd','$country','$mob','$phone')") or die(mysql_error());
      $del = mysql_query("DELETE from orders_tbl WHERE username='$username' AND row_id='$row' ") or die(mysql_error());
     }
    }
